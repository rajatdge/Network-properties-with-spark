...
def closeness(g):
	...
	# YOUR CODE HERE
	results = g.shortestPaths(landmarks=vertexList).select(functions.explode('distances')).groupBy('key').agg(functions.sum('value').alias('s')).selectExpr('key as id','1/s as closeness')
